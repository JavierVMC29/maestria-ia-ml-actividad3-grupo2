...
En el **Análisis Exploratorio (EDA)**, unimos estos dos archivos. El análisis de correlación mostró algunas relaciones entre eventos, pero la alta dimensionalidad (29 características) hizo necesaria la reducción de dimensión para la visualización. La distribución de etiquetas (que *no* se usa en el entrenamiento, solo para evaluación posterior) mostró un dataset altamente desbalanceado, con muchas más instancias "Normales" que "Anómalas", lo cual es típico en la detección de anomalías.
...
2.  **K-Means:** Se uso el método del Codo (Inercia) para determinar el número óptimo de clusters. El análisis sugirió un punto de inflexión en **k=5**, valor que se seleccionó para el modelo final.
...
Se generaron 6 gráficos comparativos clave, donde se contrastaron los clusters encontrados por K-Means (k=5) y DBSCAN contra la "verdad fundamental" (que solo se usa para visualización y evaluación, no para entrenamiento).
...
Además, se generaron tablas de resumen (medias por cluster) para interpretar el significado de cada uno de los 5 grupos de K-Means.
...
El análisis de los **5 clusters** de K-Means y los resultados de DBSCAN permitieron agrupar los comportamientos en dos perfiles *principales* (aunque k=5 permitió identificar sutilezas dentro del comportamiento normal):

* **Perfil "Normal" (Abarcando varios de los clusters de K-Means y el Cluster 0 de DBSCAN):** Representa la gran mayoría de los bloques. Se caracteriza por un alto volumen de operaciones estándar (ej. `E3`, `E26` - servicio y almacenamiento de bloques) y una baja tasa de errores. Los diferentes clusters dentro de este perfil probablemente representan ligeras variaciones del uso normal.
* **Perfil "Anómalo" (Concentrado en uno o más clusters específicos de K-Means y el "ruido" -1 de DBSCAN):** Un grupo pequeño de bloques. Se caracteriza por una frecuencia significativamente mayor de eventos de error (ej. `E20`, `E21` - errores de eliminación o excepciones).

**¿Qué diferencias clave surgieron entre los modelos?**

* **K-Means** (con **k=5**) logró un buen trabajo al forzar una partición. Fue capaz de aislar muchas de las anomalías en su propio cluster (o clusters), separándolas del comportamiento normal denso. Sin embargo, su naturaleza de buscar centros esféricos hizo que clasificara erróneamente algunos puntos normales que estaban en la "frontera".
...
    * *Solución:* Las visualizaciones son solo para *ver* la separación. La interpretación de los perfiles *debe* hacerse con la tabla de medias de las características *originales* (analizando los 5 clusters).
